...
SOURCE = PROJECT_ROOT / "Assets/Garment/Models/Sweater/Source/sweater-knit-white-diffuse_fbx_thick.fbx"
SKELETON = PROJECT_ROOT / "Tools/Blender/skeleton.json"
OUTPUT = PROJECT_ROOT / "Assets/Garment/Models/Sweater/Sweater_Rigged.fbx"

# Yoke top measured at 1.52-1.53 against a mannequin shoulder of 1.394.
GLOBAL_DROP = 0.07
...
if len(mesh_objects) != 1:
    raise RuntimeError(f"Expected one sweater mesh, found {len(mesh_objects)}")

mesh_object = mesh_objects[0]
mesh_object.data.transform(mesh_object.matrix_world)
mesh_object.matrix_world.identity()
mesh_object.name = "Sweater_Rigged"
mesh_object.data.name = "Sweater_Rigged"
...
if unweighted:
    raise RuntimeError(f"Sweater skinning left {unweighted} vertices unweighted")
